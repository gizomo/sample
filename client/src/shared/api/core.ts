...
export interface IApiMiddleware {
  name: string;
  beforeRequest?(params: RequestParams): Promise<RequestParams>;
...
export class ApiCore {
  ...
  private beforeRequest(params: RequestParams, initIndex: number = 0): Promise<RequestParams> {
      ...
      if (middleware) {
        if (middleware.beforeRequest) {
          return middleware
            ...
            .then((newParams: RequestParams) => this.beforeRequest(newParams, ++initIndex));
        }
  ...
  private request(method: RequestMethod): RequestCallback {
    return (path: string, body?: SomeObjectType): Promise<ApiResponse> =>
      this.beforeRequest({body, method, path}).then((params: RequestParams) => {
  ...
  public static create(): ApiCore {
    if (undefined === ApiCore.instance) {
      ApiCore.instance = new ApiCore({apiUrl: 'http://localhost:5000/api'});
      ApiCore.instance.appendMiddleware(new JsonContent());
      ApiCore.instance.appendMiddleware(new JwtAuth());
    }
