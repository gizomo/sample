...
import {isEmpty, normalizeUrl, toInt, toNumber} from '../../utils/helpers';
import type {ItemModel, Page} from '../../utils/pages-buffer';
import type {CastObjectType, CastReturnType, CastType, ModelType} from '.';
    ...
    switch (pathSegment) {
      ...
      case '{}':
        ...
        if (value instanceof Object && !(value instanceof Array)) {
          Object.keys(value).forEach((field: string) => (result[field] = this.castClass(path.slice(), type, value[field])));
        }
    ...
    switch (this.castRules[field] as CastType) {
      ...
      case 'datetime':
        Object.defineProperty(this, `${field}Formatted`, {
          get: () => (isEmpty(this.attributes[field]) ? undefined : (this.attributes[field] as DateTime).getFormatted()),
          set,
        });
        break;
      ...
      case 'amount':
        Object.defineProperty(this, `${field}Formatted`, {
          get: () => {
            ...
            const currency: string = this.hasAttribute('currency') && !this.attributeIsEmpty('currency') ? this.getAttribute('currency') : 'USD';
