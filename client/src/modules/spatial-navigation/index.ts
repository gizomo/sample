...
import keyboard, {KeyEvent} from '../../core/keyboard';
...
export class Navigator {
  ...
  public init(): void {
    if (!this.ready) {
      keyboard.on(KeyEvent.ON_UP, this.onKeyUp);
      keyboard.on(KeyEvent.ON_DOWN, this.onKeyDown);
  ...
  public uninit(): void {
    ...
    keyboard.off(KeyEvent.ON_UP, this.onKeyUp);
    keyboard.off(KeyEvent.ON_DOWN, this.onKeyDown);
    this.clear();
  ...
  private navigate(target: HTMLElement, direction: DirectionType, candidates: HTMLElement[], config: NavConfigType = Navigator.config): HTMLElement {
  ...
  @bind
  private onKeyDown(_keyEvent: KeyEvent, event: KeyboardEvent): boolean {
  ...
  @bind
  private onKeyUp(_keyEvent: KeyEvent, event: KeyboardEvent): void {
